...
@Component({
  ...
  templateUrl: './ubah-mhs.page.html',
  styleUrls: ['./ubah-mhs.page.scss'],
  standalone: true,
  imports: [
...
export class UbahMhsPage implements OnInit {
  ...
  mahasiswa: any; // Variabel untuk menampung data mahasiswa yang diubah
  ...
  ngOnInit() {
    ...
    if (this.idMahasiswa) {
      this.dataService.getData().then((data: any[]) => {
        this.mahasiswa = data.find(mhs => mhs.id === parseInt(this.idMahasiswa!));
        if (this.mahasiswa) {
          // Isi form dengan data this.mahasiswa yang diambil
  ...
  // Fungsi yang dipanggil saat tombol Ubah diklik
  // PERBAIKAN: tambahkankan 'async' karena penyimpanan butuh waktu
  async ubahData() {
    if (this.formMahasiswa.valid) {

      // 1. Panggil Service untuk menyimpan data
      await this.dataService.ubahData(this.idMahasiswa, this.formMahasiswa.value);

      // 2. Tampilkan pesan sukses
      console.log('Data Diubah:', this.formMahasiswa.value);
